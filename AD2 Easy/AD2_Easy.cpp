...
double AD2Easy::readAnalog(int channel) {
	double temp = 0;
...
std::tuple<bool, std::string> AD2Easy::isConnected()
{
	if (!FDwfDeviceOpen(-1, &hdwf)) {
		char error[512];
		FDwfGetLastErrorMsg(error);
...
void AD2Easy::toggleMasterSwitches(bool state)
{
	...
	FDwfAnalogOutConfigure(hdwf, -1, state);
}

void AD2Easy::toggleAnalogOut(bool state)
{
	FDwfAnalogOutConfigure(hdwf, -1, state);
}
